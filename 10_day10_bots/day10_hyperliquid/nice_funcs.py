...
def adjust_leverage_size_signal(symbol, leverage, account):
    ...
    account: LocalAccount = eth_account.Account.from_key(d.key)
    exchange = Exchange(account, constants.TESTNET_API_URL)
    info = Info(constants.TESTNET_API_URL, skip_ws=True)
...
def get_position_andmaxpos(symbol, account, max_positions):
    ...
    if len(open_positions) > max_positions:
        ...
        print(
            f"we are in {len(open_positions)} positions and max pos is {max_positions}... closing positions"
        )
        ...
        print(
            f"we are in {len(open_positions)} positions and max pos is {max_positions}... not closing positions"
        )
...
def calculate_bollinger_bands(df, length=20, std_dev=2):
    ...
    bollinger_bands = bollinger_bands.iloc[
        :, [0, 1, 2]
    ]  # Assuming the first 3 columns are BBL, BBM, and BBU
...
def get_position(symbol, account):
    ...
    user_state = info.user_state(account.address)
    print(
        f'this is the currenct account val {user_state["marginSummary"]["accountValue"]}'
    )
    positions = []
    print(f"this is the symbol {symbol}")
    for position in user_state["assetPositions"]:
...
def kill_switch(symbol, account):
    ...
    while im_in_pos:
        ...
        positions, im_in_pos, pos_size, pos_sym, entry_px, pnl_perc, long = (
...
def pnl_close(symbol, target, max_loss, account):
    ...
    if pnl_perc > target:
        print(
            f"pnl gain is {pnl_perc} and target is {target} closing position as a WIN"
        )
        kill_switch(pos_sym, account)
    elif pnl_perc <= max_loss:
        print(
            f"pnl loss is {pnl_perc} and max loss is {max_loss} closing position as a LOSS"
        )
        kill_switch(pos_sym, account)
    else:
        print(
            f"pnl loss is {pnl_perc} and max loss is {max_loss} target {target} not closing"
        )

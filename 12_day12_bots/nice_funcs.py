...
def df_sma(symbol=symbol, timeframe=timeframe, limit=limit, sma=sma):
    ...
    bars = phemex.fetch_ohlcv(symbol, timeframe=timeframe)
    # print(bars)
    df_sma = pd.DataFrame(
...
def open_positions(symbol=symbol):
    ...
    phe_bal = phemex.fetch_balance({"type": "spot", "code": "USD"})
    open_positions = phemex.fetch_positions(symbols=[symbol])
    print(open_positions)

    openpos_side = open_positions[index_pos]["info"][
        "side"
    ]  # btc [3] [0] = doge, [1] ape
    openpos_size = open_positions[index_pos]["info"]["size"]
    # print(open_positions)
...
def sleep_on_close(symbol=symbol, pause_time=pause_time):
    ...
    for ord in closed_orders[-1::-1]:
        ...
        if status == "Filled":
            ...
            print(
                f"for {symbol} this is the time {txttime} this is the orderstatus {status}"
            )
...
def ob(symbol=symbol, vol_repeat=vol_repeat, vol_time=vol_time):
    ...
    print(
        f"last {mins}mins for {symbol} this is total Bid Vol: {total_bidvol} | ask vol: {total_askvol}"
    )
...
def pnl_close(symbol, target, max_loss):
    ...
    pos_dict = phemex.fetch_positions(symbols=symbols)
    # print(pos_dict)
